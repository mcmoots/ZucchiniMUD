...
void show_char_to_char_0 (CHAR_DATA * victim, CHAR_DATA * ch)
{
    ...
    if (IS_SET (victim->comm, COMM_AFK))
        strcat (buf, "{c[{CAFK{c]{x ");
    if (IS_AFFECTED (victim, AFF_INVISIBLE))
        strcat (buf, "{D(Invis){x ");
    if (victim->invis_level >= LEVEL_HERO)
        strcat (buf, "{g({GWizi{g){x ");
    if (IS_AFFECTED (victim, AFF_HIDE))
        strcat (buf, "{D(Hide){x ");
    if (IS_AFFECTED (victim, AFF_CHARM))
        strcat (buf, "{m({MCharmed{m){x ");
    if (IS_AFFECTED (victim, AFF_PASS_DOOR))
        strcat (buf, "{D(Translucent){x ");
    ...
    if (!IS_NPC (victim) && IS_SET (victim->act, PLR_KILLER))
        strcat (buf, "{r({RKILLER{r){x ");
    if (!IS_NPC (victim) && IS_SET (victim->act, PLR_THIEF))
        strcat (buf, "{y({YTHIEF{y){x ");
...
void do_autogold (CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char ("Automatic cash looting set.\n\r", ch);
...
void do_autosplit (CHAR_DATA * ch, char *argument)
{
    ...
    {
        send_to_char ("Automatic cash splitting set.\n\r", ch);
...
void do_look (CHAR_DATA * ch, char *argument)
{
    ...
    if (!IS_NPC (ch)
        && !IS_SET (ch->act, PLR_HOLYLIGHT) && !IS_AFFECTED(ch, AFF_DARK_VISION) && room_is_dark (ch->in_room))
    {
    ...
    if ((obj = get_obj_here (ch, arg)) != NULL)
    {
        switch (obj->item_type)
        {
            ...
            case ITEM_MONEY:
                ...
                {
                   sprintf (buf, "Wow. One zloty.\n\r");
                }
                else
                    sprintf (buf, "There are %d zlotys in the pile.\n\r", obj->value[0]);
                send_to_char (buf, ch);
                break;
...
void do_worth (CHAR_DATA * ch, char *argument)
{
    ...
    if (IS_NPC (ch))
    {
        sprintf (buf, "You have %ld %s cash.\n\r",
                 ch->cash, ch->cash > 1 ? "zlotys" : "zloty");
        send_to_char (buf, ch);
        return;
    }

    sprintf (buf,
             "You have %ld %s cash, and %d experience (%d exp to level).\n\r",
             ch->cash, ch->cash > 1 ? "zlotys" : "zloty", ch->exp,
...
void do_score (CHAR_DATA * ch, char *argument)
{
    ...
    sprintf (buf, "{gName:{x %s%s\n\r", ch->name, IS_NPC (ch) ? "" : ch->pcdata->title);
    send_to_char(buf, ch);

    send_to_char("{g---------------------------------------------------------------------\n\r{x", ch);
	       
    if (get_trust (ch) != ch->level)
        sprintf (buf, "{gLevel:{x %2d ({W%2d{x)        ", ch->level, get_trust (ch));
    else
    	sprintf( buf, "{gLevel:{x %2d             ", ch->level);

    send_to_char (buf, ch);

    sprintf( buf, "{gAge:{x %d (%d hours)\n\r",
    		get_age(ch), (ch->played + (int) (current_time - ch->logon)) / 3600 );
    send_to_char(buf, ch);

    sprintf (buf, "{gRace:{x  %-13s  {gCarrying:{x {W%-d{x/{W%d{x items and {W%d{x/{W%d{x kg\n\r",
             race_table[ch->race].name, ch->carry_number, can_carry_n(ch),
	     get_carry_weight(ch)/10, can_carry_w(ch)/10 );
    send_to_char (buf, ch);

    sprintf( buf, "{gSex:{x   %-14s {gExperience:{x %d ({W%-d{x to next level)\n\r", 
             ch->sex == 0 ? "sexless" : ch->sex == 1 ? "male" : "female",
	     ch->exp, IS_NPC(ch) ? 0 : ((ch->level + 1) * exp_per_level (ch, ch->pcdata->points) - ch->exp));
    send_to_char(buf, ch);
    
    send_to_char("{g---------------------------------------------------------------------\n\r{x", ch);
    
    sprintf( buf, "{gHit:{x   %5d/%-5d    {gWimpy:{x   %-d hit points\n\r",
    		ch->hit, ch->max_hit, ch->wimpy );
    send_to_char( buf, ch );

    sprintf(buf, "{gMana:{x  %5d/%-5d    {gHitroll:{x %-d\n\r",
    		ch->mana, ch->max_mana, GET_HITROLL(ch) );
    send_to_char(buf, ch);
    
    sprintf(buf, "{gMove:{x  %5d/%-5d    {gDamroll:{x %-d\n\r",
    		ch->move, ch->max_move, GET_DAMROLL(ch));
    send_to_char(buf, ch);
    
    send_to_char("{g---------------------------------------------------------------------\n\r{x", ch);

    sprintf( buf, "{gStr:{x   %-2d ({W%2d{x)        {gArmor{x\n\r",
    	     ch->perm_stat[STAT_STR], get_curr_stat (ch, STAT_STR) );
    send_to_char(buf, ch);
    
    sprintf( buf, "{gInt:{x   %-2d ({W%2d{x)        {gBash:{x   %-d\n\r",
             ch->perm_stat[STAT_INT], get_curr_stat(ch, STAT_INT), GET_AC(ch, AC_BASH) );
    send_to_char(buf, ch);
    
    sprintf( buf, "{gWis:{x   %-2d ({W%2d{x)        {gPierce:{x %-d\n\r",
    		ch->perm_stat[STAT_WIS], get_curr_stat(ch, STAT_WIS), GET_AC(ch, AC_PIERCE) );
    send_to_char(buf, ch);
    
    sprintf( buf, "{gDex:{x   %-2d ({W%2d{x)        {gSlash:{x  %-d\n\r",
    		ch->perm_stat[STAT_DEX], get_curr_stat(ch, STAT_DEX), GET_AC(ch, AC_SLASH) );
    send_to_char(buf, ch);
    
    sprintf( buf, "{gCon:{x   %-2d ({W%2d{x)        {gMagic:{x  %-d\n\r", 
    		ch->perm_stat[STAT_CON], get_curr_stat(ch, STAT_CON), GET_AC(ch, AC_EXOTIC) );
    send_to_char(buf, ch);
    
    send_to_char("{g---------------------------------------------------------------------\n\r{x", ch);

    sprintf (buf, "{gCash:{x     %-5ld       {gQuest points:{x   %-d\n\r",
             ch->cash, IS_NPC(ch) ? 0 : ch->pcdata->quest_curr);
    send_to_char (buf, ch);
    
    sprintf( buf, "{gBank:{x     %-5ld       {gBrownie points:{x %-d\n\r", 
    		IS_NPC(ch) ? 0 : ch->pcdata->balance, IS_NPC(ch) ? 0 : ch->pcdata->browniepoints);
    send_to_char(buf, ch);
    
    sprintf( buf, "{gTrain:{x    %-2d          {gAlignment:{x       %d\n\r",
    		ch->train, ch->alignment);
    send_to_char(buf, ch);
    
    sprintf( buf, "{gPractice:{x %-2d          {gGod:{x %s %s\n\r", ch->practice,
             IS_NPC(ch) ? "Mota" : god_table[ch->pcdata->god].name,
	     IS_NPC(ch) ? "the Generic" : god_table[ch->pcdata->god].title );
    send_to_char(buf, ch);
...
char *const day_name[] = {
    "Dsfargeg", "the Kuiper Belt", "Libations", "the Oort Cloud", "Extra Naps",
    "Watering the Plants", "Eris"
};

char *const month_name[] = {
    "Roflcopter", "Nostalgia", "Dandelions", "the Pulley",
    "the Grand Fluff", "the Spring", "Untember", "Futility", "Extra Breakfasts",
    "Snickerdoodles", "the Yellow Face", "the Inclined Plane", "Bogosity", "Dismember",
    "Soup", "Kitteh", "Lollerskates"
};
...
void do_time (CHAR_DATA * ch, char *argument)
{
    ...
    send_to_char (buf, ch);
    sprintf (buf, "ROM started up at %sThe system time is %s",
...
void do_help (CHAR_DATA * ch, char *argument)
{
    ...
    for (pHelp = help_first; pHelp != NULL; pHelp = pHelp->next)
    {
	    ...
	    {
	    	send_to_char("\n\r", ch);
		send_to_char("More than one help file was found.\n\r", ch);
                send_to_char("{g========================================================================{x\n\r", ch);
		...
		for( pHelp = help_first; pHelp != NULL; pHelp = pHelp->next )
		{
			...
			if( is_name( argall, pHelp->keyword ) && pHelp->level >= -1 && str_cmp( argall, "imotd" ))
			{
				count++;
		...
		return;
            ...
            found = TRUE;
            /* small hack :) */
            if (ch->desc != NULL && ch->desc->connected != CON_PLAYING && ch->desc->connected != CON_GET_NEW_RACE
	        && ch->desc->connected != CON_GET_GOD)
                break;
        }
    }
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        ...
        if (!str_prefix (arg, wch->name))
        {
            ...
            sprintf (buf, "[%2d %6s] %s%s%s%s%s%s%s%s\n\r",
                     ...
                     wch->invis_level >= LEVEL_HERO ? "{D(Wizi){x " : "",
                     clan_table[wch->clan].who_name, IS_SET (wch->comm,
                                                             COMM_AFK) ?
                     "[AFK] " : "", IS_SET (wch->act,
                                            PLR_KILLER) ? "{r(KILLER){x " : "",
                     IS_SET (wch->act, PLR_THIEF) ? "{y(THIEF){x " : "",
...
void do_who (CHAR_DATA * ch, char *argument)
{
                  ...
                  if (!str_prefix (arg, "clan"))
                     ...
                     {  
                       ...
                       if (iClan)
                          ...
                          {
                           send_to_char("That's not a valid race or clan.\n\r", ch);
                           return;
                          }
                     }
              }
	...
	{
		if(colorstrlen(wch->pcdata->immtitle) == 8)
			sprintf(buf3, "%s", wch->pcdata->immtitle);
		else if( colorstrlen(wch->pcdata->immtitle) == 7)
			sprintf(buf3, "%s ", wch->pcdata->immtitle);
		else
		{
			half = ((8 - colorstrlen(wch->pcdata->immtitle)))/2;
			sechalf = (8 - (half + colorstrlen(wch->pcdata->immtitle)));
			sprintf(buf3, "%*c%s%*c", half, ' ', wch->pcdata->immtitle, sechalf, ' ');
		}
	}
	else
		sprintf(buf3, "%2d %s", wch->level,
			wch->race < MAX_PC_RACE ? pc_race_table[wch->race].who_name : "     ");

        sprintf (buf, "[%s] %s%s%s%s%s%s%s%s%s\n\r", buf3,
		 IS_SET (wch->act, PLR_RP) ? "{g(RP){x " : "",
                 wch->incog_level >= LEVEL_HERO ? "{b(Incog){x " : "",
                 wch->invis_level >= LEVEL_HERO ? "{D(Wizi){x " : "",
                 clan_table[wch->clan].who_name,
                 IS_SET (wch->comm, COMM_AFK) ? "{c(AFK){x " : "",
...
void do_practice (CHAR_DATA * ch, char *argument)
{
    ...
    else
    {
        ...
        adept = IS_NPC (ch) ? 100 : skill_table[sn].skill_adept;
