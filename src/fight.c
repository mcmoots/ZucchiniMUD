...
void multi_hit (CHAR_DATA * ch, CHAR_DATA * victim, int dt)
{
    ...
    chance = 3 * get_skill (ch, gsn_third_attack) / 4;
	...
	chance = 2 * get_skill (ch, gsn_fourth_attack) / 3;
             ...
             int dam_type, bool show)
{
    ...
    switch (victim->position)
    {
        ...
        case POS_DEAD:
            act ("{r$n is {RDEAD!!{x", victim, 0, 0, TO_ROOM);
            send_to_char ("{rYou have been {RKILLED!!{x\n\r\n\r", victim);
            break;

        default:
            if (dam > victim->max_hit / 4)
                send_to_char ("{rThat really did {RHURT!{x\n\r", victim);
...
void make_corpse (CHAR_DATA * ch)
{
    ...
    if (IS_NPC (ch))
    {
        ...
        if (ch->cash > 0)
        {
            obj_to_obj (create_money (ch->cash), corpse);
            ch->cash = 0;
        }
        corpse->cost = 0;
    }
    else
    {
        ...
        else
        {
            ...
            if (ch->cash > 1)
            {
                obj_to_obj (create_money (ch->cash / 2), corpse);
                ch->cash -= ch->cash / 2;
            }
        }
	...
	/* Poisonous mobs leave poisonous corpses */
...
void death_cry (CHAR_DATA * ch)
{
    ...
    switch (number_range (0, 12))
    {
		...
		case 7:
			...
			{
				if( number_range(0,2) && (IS_AFFECTED(ch, AFF_DETECT_EVIL) || IS_AFFECTED(ch, AFF_DETECT_GOOD) || IS_AFFECTED(ch, AFF_DETECT_HIDDEN) || IS_AFFECTED(ch, AFF_DETECT_MAGIC) || IS_AFFECTED(ch, AFF_INFRARED) || IS_AFFECTED(ch, AFF_DETECT_INVIS) || IS_AFFECTED(ch, AFF_DARK_VISION)) )/* load eye as pill */
				{
					if( IS_SET(ch->parts, PART_EYESTALKS))
						{ msg = "$n's eyeball is sliced from its stalk."; }
					else { msg = "$n's eyeball flies out of its socket."; }
					vnum = OBJ_VNUM_EYE_PILL;
				}
				else {
					if( IS_SET(ch->parts, PART_EYESTALKS))
						{ msg = "$n's eyeball is sliced from its stalk."; }
					else { msg = "$n's eyeball flies out of its socket."; }
					vnum = OBJ_VNUM_EYE_FOOD;
				}
			}
			break;
		case 8:
		...
		case 11:
			...
			{
				msg = "$n's wing is sliced from $s dead body.";
				vnum = OBJ_VNUM_SLICED_WING;
			}
			break;
		case 12:
			if( IS_SET (ch->parts, PART_HORNS))
			{
				msg = "$n's horns are torn from $s dead body.";
				vnum = OBJ_VNUM_HORNS;
			}
			break;
    }
    ...
    if (vnum != 0)
    {
        ...
        if (vnum == OBJ_VNUM_EYE_PILL)
        {
        	...
        	$i = (IS_AFFECTED(ch, AFF_DETECT_EVIL) > 0) + (IS_AFFECTED(ch, AFF_DETECT_INVIS) > 0) + (IS_AFFECTED(ch, AFF_DETECT_MAGIC) > 0) + (IS_AFFECTED(ch, AFF_DETECT_GOOD) > 0) + (IS_AFFECTED(ch, AFF_DETECT_HIDDEN) > 0) + (IS_AFFECTED(ch, AFF_INFRARED) > 0) + (IS_AFFECTED(ch, AFF_DARK_VISION) > 0) + (IS_AFFECTED(ch, AFF_BLIND) > 0);
 				...
 				while( $j < 4 )
 				{
 					switch(number_range(0,7))
 					{
 						case 0:
 							...
 							{ obj->value[$j] = skill_lookup("detect evil"); $j++; }
 							break;
						case 1:
							if( IS_AFFECTED(ch, AFF_DETECT_GOOD))
							{ obj->value[$j] = skill_lookup("detect good"); $j++; }
							break;
						case 2:
							if( IS_AFFECTED(ch, AFF_DETECT_INVIS))
							{ obj->value[$j] = skill_lookup("detect invisible"); $j++; }
							break;
						case 3:
							if( IS_AFFECTED(ch, AFF_DETECT_HIDDEN))
							{ obj->value[$j] = skill_lookup("detect hidden"); $j++; }
							break;
						case 4:
							if( IS_AFFECTED(ch, AFF_DETECT_MAGIC))
							{ obj->value[$j] = skill_lookup("detect magic"); $j++; }
							break;
						case 5:
							if( IS_AFFECTED(ch, AFF_INFRARED))
							{ obj->value[$j] = skill_lookup("infravision"); $j++; }
							break;
						case 6:
							if( IS_AFFECTED(ch, AFF_DARK_VISION))
							{ obj->value[$j] = skill_lookup("cure blindness"); $j++; }
							break;
						case 7:
							if( IS_AFFECTED(ch, AFF_BLIND))
							{ obj->value[$j] = skill_lookup("blindness"); $j++; }
							break;
					}
 				}
 			}
 			...
 			else if( $i > 0 )
 			{
 				...
 				if( IS_AFFECTED(ch, AFF_INFRARED) )
 				{ obj->value[$j] = skill_lookup("infravision"); $j++; }
 				if( IS_AFFECTED(ch, AFF_DARK_VISION) )		/* Need a dark vision spell */
 				{ obj->value[$j] = skill_lookup("cure blindness"); $j++; }
 				if( IS_AFFECTED(ch, AFF_BLIND) )
 				{ obj->value[$j] = skill_lookup("blindness"); $j++; }
 			}
		...
		if ( IS_NPC(ch) && obj->item_type == ITEM_BRAINS )
		{
			obj->value[0] = skill_lookup(ch->brain_content);
		}
...
*/
	...
	if(!IS_NPC(gch) && IS_SET(gch->act,PLR_QUESTING) && IS_NPC(victim))
	{
		if(gch->pcdata->questmob == victim->pIndexData->vnum)
		{
			send_to_char("You have almost completed your quest!\n\r", gch);
			send_to_char("Return to the questmaster before you run out of time.\n\r", gch);
                  ...
                  bool immune)
{
    ...
    {
        vs = "{Y={R={Y= {ROBLITERATE {Y={R={Y=";
        vp = "{Y={R={Y= {ROBLITERATES {Y={R={Y=";
    }
    else if (dam_percent <= 90)
    {
        vs = "{T   {W ANNIHILATE {T   {x ";
        vp = "{T   {W ANNIHILATES {T   {x";
    }
    ...
    {
        vs = "{udo UNSPEAKABLE things to{x";
        vp = "{udoes UNSPEAKABLE things to{x";
    }
    ...
    if (dt == TYPE_HIT)
    {
        if (ch == victim)
        {
            sprintf (buf1, "{y$n %s {y$melf%c {y[{Y%d{y]{x", vp, punct, dam);
            sprintf (buf2, "{rYou %s {ryourself%c {r[{R%d{r]{x", vs, punct, dam);
        }
        else
        {
            sprintf (buf1, "{y$n %s {y$N%c {y[{Y%d{y]{x", vp, punct, dam);
            sprintf (buf2, "{gYou %s {g$N%c {g[{G%d{g]{x", vs, punct, dam);
            sprintf (buf3, "{r$n %s {ryou%c {r[{R%d{r]{x", vp, punct, dam);
        }
    }
    else
    {
        ...
        if (immune)
        {
            if (ch == victim)
            {
                sprintf (buf1, "{Y$n is unaffected by $s own %s.{x", attack);
                sprintf (buf2, "{GLuckily, you are immune to that.{x");
            }
            else
            {
                sprintf (buf1, "{Y$N is unaffected by $n's %s!{x", attack);
                sprintf (buf2, "{G$N is unaffected by your %s!{x", attack);
                sprintf (buf3, "{R$n's %s is powerless against you.{x",
                         attack);
            }
        }
        else
        {
            if (ch == victim)
            {
                sprintf (buf1, "{y$n's %s %s{y $m%c [{Y%d{y]{x", attack, vp, punct, dam);
                sprintf (buf2, "{rYour %s %s{r you%c [{R%d{r]{x", attack, vp, punct, dam);
            }
            else
            {
                sprintf (buf1, "{y$n's %s %s{y $N%c[{Y%d{y]{x", attack, vp, punct, dam);
                sprintf (buf2, "{gYour %s %s{g $N%c [{G%d{g]{x", attack, vp, punct, dam);
                sprintf (buf3, "{r$n's %s %s{r you%c [{R%d{r]{x", attack, vp, punct, dam);
            }
        }
    }
...
void do_berserk (CHAR_DATA * ch, char *argument)
{
    ...
    if ((chance = get_skill (ch, gsn_berserk)) == 0
        ...
        || (!IS_NPC (ch)
            && ch->level < skill_table[gsn_berserk].skill_level))
    {
    ...
    if (number_percent () < chance)
    {
        ...
        send_to_char ("{5Your pulse races as you are consumed by rage!{x\n\r",
                      ch);
        act ("{5$n gets a wild look in $s eyes.{x", ch, NULL, NULL, TO_ROOM);
...
void do_bash (CHAR_DATA * ch, char *argument)
{
    ...
    if ((chance = get_skill (ch, gsn_bash)) == 0
        ...
        || (!IS_NPC (ch)
            && ch->level < skill_table[gsn_bash].skill_level))
    {
...
void do_tackle (CHAR_DATA * ch, char *argument)
{
    ...
    if ((chance = get_skill (ch, gsn_tackle)) == 0
        || (IS_NPC (ch) && !IS_SET (ch->off_flags, OFF_BASH))
        || (!IS_NPC (ch)
            && ch->level < skill_table[gsn_tackle].skill_level))
    {
        send_to_char ("Leave the tackles to the football team.\n\r", ch);
        return;
    }
    ...
    if (victim == ch)
    {
        send_to_char ("You try to tackle, but fail.\n\r", ch);
        return;
    }
	...
	dam = number_range(4, 4 + 3 * ch->size + chance / 20);
	
	sprintf(buf, "{w$n sends you to the ground with a powerful tackle! [{W%d{w]{x", dam);
        act (buf, ch, NULL, victim, TO_VICT);
	sprintf(buf, "{wYou tackle $N, and send $M flying! [{W%d{w]{x", dam);
        act (buf, ch, NULL, victim, TO_CHAR);
	sprintf(buf, "{w$n sends $N to the ground with a powerful tackle. [{W%d{w]{x", dam);
        act (buf, ch, NULL, victim, TO_NOTVICT);
        check_improve (ch, gsn_tackle, TRUE, 1);
	
	daze = number_range(2, 4);

        DAZE_STATE (victim, daze * PULSE_VIOLENCE);
        WAIT_STATE (ch, skill_table[gsn_tackle].beats);
        victim->position = POS_RESTING;
        damage (ch, victim, dam, gsn_tackle, DAM_BASH, FALSE);

    }
    else
    {
        damage (ch, victim, 0, gsn_tackle, DAM_BASH, FALSE);
        act ("{wYou fall flat on your face!{x", ch, NULL, victim, TO_CHAR);
        act ("{w$n falls flat on $s face.{x", ch, NULL, victim, TO_NOTVICT);
        act ("{wYou evade $n's bash, causing $m to fall flat on $s face.{x",
             ch, NULL, victim, TO_VICT);
        check_improve (ch, gsn_tackle, FALSE, 1);
        ch->position = POS_RESTING;
        WAIT_STATE (ch, skill_table[gsn_tackle].beats * 3 / 2);
    }
    check_killer (ch, victim);
}
...
void do_dirt (CHAR_DATA * ch, char *argument)
{
    ...
    if ((chance = get_skill (ch, gsn_dirt)) == 0
        ...
        || (!IS_NPC (ch)
            && ch->level < skill_table[gsn_dirt].skill_level))
    {
...
void do_trip (CHAR_DATA * ch, char *argument)
{
    ...
    if ((chance = get_skill (ch, gsn_trip)) == 0
        ...
        || (!IS_NPC (ch)
            && ch->level < skill_table[gsn_trip].skill_level))
    {
...
void do_flee (CHAR_DATA * ch, char *argument)
{
    ...
    for (attempt = 0; attempt < 6; attempt++)
    {
	    ...
	    /* there was a check for thieves to flee without losing xp - duplicate this somehow? */
