...
void say_spell (CHAR_DATA * ch, int sn)
{
    ...
    {
        if (rch != ch)
            act ((!IS_NPC (rch) && ch->level <= rch->level) ? buf : buf2,
                 ch, NULL, rch, TO_VICT);
    }
...
void do_cast (CHAR_DATA * ch, char *argument)
{
	...
	/* Earwormed chars must make a saving roll to cast */
	if( is_affected(ch, gsn_earworm) ){
		
		cancast = FALSE;
		save = 50 + victim->saving_throw *2;
		
		if( IS_AFFECTED(victim, AFF_BERSERK) )
			save += ch->level / 2;
		
		switch(check_immune(ch, DAM_MENTAL)){
			case IS_IMMUNE:
				cancast = TRUE;
				break;
			case IS_RESISTANT:
		...
		if( !cancast ){
			act("$n sings a snippet of the Smurf theme song.", ch, NULL, NULL, TO_ROOM);
			send_to_char("You try to cast the spell, but find yourself singing the Smurf theme instead.\n\r", ch);
			return;
		}
	}

    if (ch->level + 2 == skill_table[sn].skill_level)
        mana = 50;
    else
                         ...
                         int target)
{
	...
	if(check_dispel (level, victim, skill_lookup("night vision")))
		found = TRUE;
                         ...
                         int target)
{
    ...
    {
    	send_to_char("You can't get your victim's attention.\n\r", ch);
...
void spell_night_vision (int sn, int level, CHAR_DATA * ch, void *vo,
                        int target)
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    AFFECT_DATA af;
    ...
    if (IS_AFFECTED (victim, AFF_DARK_VISION))
    {
        if (victim == ch)
            ...
            act ("$N already has night vision.\n\r", ch, NULL, victim,
                 TO_CHAR);
        return;
    }
    act ("$n's eyes glow green.\n\r", ch, NULL, NULL, TO_ROOM);
    ...
    affect_to_char (victim, &af);
    send_to_char ("Your eyes glow green.\n\r", victim);
    return;
}
