...
void do_get (CHAR_DATA * ch, char *argument)
{
    ...
    if (arg2[0] == '\0')
    {
	    ...
	    if( ch->in_room->vnum == ROOM_VNUM_DONATE && !IS_IMMORTAL(ch))
	    {
...
void do_donate (CHAR_DATA *ch, char *argument)
{
	...
	if( !CAN_WEAR( obj, ITEM_TAKE ) || IS_OBJ_STAT(obj, ITEM_MELT_DROP) )
	{
		send_to_char("The Salvation Army doesn't want that either.\n\r", ch);
		return;
	}	
	
	obj_from_char( obj );
	obj_to_room( obj, get_room_index(ROOM_VNUM_DONATE) );
	
        act ("$n gives $p to the Salvation Army.", ch, obj, NULL, TO_ROOM);
...
void do_drop (CHAR_DATA * ch, char *argument)
{
	...
	int kash = 0;
        ...
        argument = one_argument (argument, arg);
        if (amount <= 0
            || (str_cmp (arg, "money") && str_cmp (arg, "zlotys") &&
                str_cmp (arg, "zloty") && str_cmp (arg, "cash")))
        {
            send_to_char ("Sorry, you can't do that.\n\r", ch);
            return;
        }

        if (!str_cmp (arg, "money") || !str_cmp (arg, "cash")
            || !str_cmp (arg, "zloty") || !str_cmp( arg, "zlotys" ))
        {
            if (ch->cash < amount)
            {
        ...
        obj_to_room (create_money (kash), ch->in_room);
...
void do_give (CHAR_DATA * ch, char *argument)
{
    ...
    if (is_number (arg1))
    {
        ...
        amount = atoi (arg1);
        if (amount <= 0
            || (str_cmp (arg2, "cash") && str_cmp (arg2, "money") &&
                str_cmp (arg2, "zloty") && str_cmp (arg2, "zlotys")))
        {
            send_to_char ("Sorry, you can't do that.\n\r", ch);
            return;
        }
...
void do_eat (CHAR_DATA * ch, char *argument)
{
    ...
    if (!IS_IMMORTAL (ch))
    {
        if (obj->item_type != ITEM_FOOD && obj->item_type != ITEM_PILL && obj->item_type != ITEM_BRAINS)
        {
    ...
    switch (obj->item_type)
    {
        ...
        case ITEM_FOOD:
            ...
            if (obj->value[3] != 0)
            {
				...
				/* check to see if it's rejected */
				if( !IS_NPC(ch) && number_percent() <= ch->pcdata->learned[gsn_vomit] )
				{
					send_to_char("That didn't agree with you so well... your gut heaves, and you vomit.\n\r", ch);
					act("$n vomits up $p.", ch, obj, NULL, TO_ROOM);
		...
		case ITEM_BRAINS:
			if( IS_NPC(ch))
			{
				send_to_char("NPCs don't get to eat brains.\n\r", ch);
				return;
			}
			...
			else if ( skill_table[obj->value[0]].skill_level > ch->level )
			{
				send_to_char("The power in these brains is too strong for you to absorb.\n\r", ch);
			}
			else if (ch->pcdata->learned[obj->value[0]] < 1 )
			{
				ch->pcdata->learned[obj->value[0]] = 1;
				sprintf(buf, "You have learned the art of %s!\n\r", skill_table[obj->value[0]].name);
				send_to_char(buf, ch);
			}
			else if( ch->pcdata->learned[obj->value[0]] >= 1  && ch->pcdata->learned[obj->value[0]] < skill_table[obj->value[0]].skill_adept)
			{
				ch->pcdata->learned[obj->value[0]] += number_range(1, 10);
				sprintf(buf, "Your skill at %s has improved.\n\r", skill_table[obj->value[0]].name);
				send_to_char(buf, ch);
			}
			else
			{
...
void do_sacrifice (CHAR_DATA * ch, char *argument)
{
    ...
    money = UMAX (1, obj->level * 3);

    if (obj->item_type != ITEM_CORPSE_NPC && obj->item_type != ITEM_CORPSE_PC)
        money = UMIN (money, obj->cost);

    sprintf (buf, "%s %s gives you %d zloty%s for your sacrifice.\n\r", 
             IS_NPC(ch) ? "Mota" : god_table[ch->pcdata->god].name, 
...
void do_steal (CHAR_DATA * ch, char *argument)
{
        ...
        switch (number_range (0, 3))
        {
            ...
            case 1:
                sprintf (buf, "%s couldn't rob %s way out of a paper bag!",
                         ch->name, (ch->sex == 2) ? "her" : (ch->sex == 1) ? "his" : "its");
                break;
            case 2:
    ...
    if (!str_cmp (arg1, "cash")
        || !str_cmp (arg1, "money")
        || !str_cmp (arg1, "zloty") || !str_cmp (arg1, "zlotys"))
    {
        int amount;

        amount = victim->cash * number_range (1, ch->level) / MAX_LEVEL;
        if (amount <= 0)
        {
        ...
        ch->cash += amount;
        victim->cash -= amount;
        sprintf (buf, "Bingo!  You got %d %s.\n\r", amount, amount == 1 ? "zloty" : "zlotys");
...
void do_buy (CHAR_DATA * ch, char *argument)
{
    ...
    else
    {
        ...
        if (number > 1)
        {
            ...
            sprintf (buf, "You buy $p[%d] for %d zlotys.", number,
                     cost * number);
        ...
        else
        {
            ...
            sprintf (buf, "You buy $p for %d %s.", cost, cost == 1 ? "zloty" : "zlotys");
            act (buf, ch, obj, NULL, TO_CHAR);
        }
        deduct_cost (ch, cost * number);
        keeper->cash += cost * number;
...
void do_sell (CHAR_DATA * ch, char *argument)
{
        ...
        && roll < get_skill (ch, gsn_haggle))
    {
        ...
        cost = UMIN (cost, (keeper->cash));
        check_improve (ch, gsn_haggle, TRUE, 4);
    }
    sprintf (buf, "You sell $p for %d %s.", cost, cost == 1 ? "zloty" : "zlotys");
    act (buf, ch, obj, NULL, TO_CHAR);
    ch->cash += cost;
...
void do_value (CHAR_DATA * ch, char *argument)
{
    ...
    sprintf (buf,
             "$n tells you 'I'll give you %d %s for $p'.",
             cost, cost > 1 ? "zlotys" : "zloty");
...
void do_butcher( CHAR_DATA *ch, char *argument )
{
	...
	if( number_percent() < get_skill(ch, gsn_butcher)){
		...
		for(i = 0; i < nsteaks; i++){
			...
			/* Poisonous corpses make poisonous steaks */
			if( obj->value[3] != 0 ){
		...
		buf[0] = '\0';
		sprintf(buf, "$n butchers %s.\n\r", obj->short_descr);
		act(buf, ch, NULL, NULL, TO_ROOM);
		
		sprintf(buf, "You butcher %s and create %d steak%s.\n\r", obj->short_descr, nsteaks, nsteaks > 1 ? "s" : "");
		send_to_char(buf, ch);
	}
	
	else{
		sprintf(buf, "$n chops %s into tiny, useless pieces.\n\r", obj->short_descr);
		act(buf, ch, NULL, NULL, TO_ROOM);
		send_to_char("You destroy the corpse.\n\r", ch);
	}
	
	check_improve(ch, gsn_butcher, TRUE, 1);
	
	/*Dump corpse inventory to room*/
...
void do_vomit( CHAR_DATA *ch, char *argument )
{
  if( IS_NPC(ch))
    {
      send_to_char("You empty your stomach.\n\r", ch);
      act("$n vomits.", ch, NULL, NULL, TO_ROOM);
      ch->pcdata->condition[COND_FULL] = 0;
      return;
    }

  if( number_percent() <= ch->pcdata->learned[gsn_vomit] )
    {
      send_to_char("You empty your stomach, and feel much better.\n\r",
ch);
      act("$n vomits.", ch, NULL, NULL, TO_ROOM);
      ch->pcdata->condition[COND_FULL] = 0;
    }
    ...
    {
      send_to_char("Your gut heaves, but nothing happens.\n\r", ch);
    }
...
void do_balance( CHAR_DATA *ch, char *argument )
{
	...
	if( IS_NPC(ch))
	{
		send_to_char("NPCs don't have bank accounts.\n\r", ch);
		return;
	}
	...
	if( banker == NULL || !IS_SET(banker->act, ACT_BANKER))
	{
		send_to_char("You can't do that here.\n\r", ch);
		return;
	}
	
	if( banker->fighting != NULL )
	{
		send_to_char("Wait until the fighting stops.\n\r", ch);
		return;
	}
	
	sprintf(buf, "{k%s tells you '{KYou have %ld zloty%s in the bank.{k'{x\n\r",
		capitalize(PERS(banker, ch)), ch->pcdata->balance, ch->pcdata->balance == 1 ? "" : "s");
	send_to_char(buf, ch);
}
...
void do_deposit( CHAR_DATA *ch, char *argument)
{
	...
	sprintf(buf, "You deposit %d zloty%s.\n\r", value, value == 1 ? "" : "s");
	send_to_char(buf, ch);
	
	sprintf(buf, "Your balance is now %ld.\n\r", ch->pcdata->balance);
	send_to_char(buf, ch);
}
...
void do_withdraw (CHAR_DATA *ch, char *argument)
{
	...
	{
		send_to_char("That would overdraw your account.\n\r", ch);
		return;
	}
	...
	sprintf(buf, "You withdraw %d zloty%s.\n\r", value, value == 1 ? "" : "s");
	send_to_char(buf, ch);
