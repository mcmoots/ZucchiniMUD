...
REDIT( redit_mplist )
{
	...
	one_argument(argument, arg);
	if(arg[0] == '\0'){
		send_to_char("Syntax: mplist <all/name>\n\r", ch);
		return FALSE;
	}
	...
	for (vnum = pArea->min_vnum; vnum <= pArea->max_vnum; vnum++){
		if( (pMcode = get_mprog_index(vnum)) != NULL && (fAll || is_name(arg, pMcode->name) ) ){
			found = TRUE;
			sprintf(buf, "[%5d] %-17.16s", pMcode->vnum, pMcode->name);
			add_buf(buf1, buf);
			if(++col %3 == 0){
				add_buf(buf1, "\n\r");
			}
	...
	if(!found){
		send_to_char("Mprog(s) not found in this area.\n\r", ch);
		return FALSE;
	}
...
AEDIT (aedit_level)
{
	...
	if( (ilower = atoi(lower)) > (iupper = atoi(upper)) )
	{
		send_to_char( "AEdit: Upper level must be larger than lower level.\n\r", ch);
		return FALSE;
	}
...
void show_obj_values (CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
	...
	case ITEM_BRAINS:
		sprintf( buf, "[v0] Knowledge:   %s\n\r", obj->value[0] != -1 ? skill_table[obj->value[0]].name : "none");
		send_to_char(buf, ch);
		break;
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
		...
		case ITEM_CORPSE_PC:
			sprintf( buf, "[v3] Poisonous: %s\n\r", obj->value[3] !=0 ? "Yes" : "No");
			send_to_char(buf, ch);
			break;

        case ITEM_MONEY:
            sprintf (buf, "[v0] Zlotys:   [%d]\n\r", obj->value[0]);
            send_to_char (buf, ch);
            break;
    }
                     ...
                     char *argument)
{
    switch (pObj->item_type)
    {
		...
		case ITEM_BRAINS:
			...
			{
				default:
					do_help( ch, "ITEM_BRAINS");
					return FALSE;
				case 0:
					send_to_char( "BRAIN CONTENTS SET.\n\r\n\r", ch);
...
MEDIT (medit_brain_content)
{
	...
	if( argument[0] == '\0')
	{
		send_to_char( "Syntax: brain [skill/spell]\n\r", ch);
		return FALSE;
	}
	
	if( skill_lookup(argument) <= 0 )
	{
		send_to_char( "Warning: that's not a current skill or spell!\n\r", ch);
	}
	
	free_string( pMob->brain_content);
	pMob->brain_content = str_dup( argument );
	
	send_to_char( "Brain set.\n\r", ch );
	return TRUE;
}
