...
void advance_level (CHAR_DATA * ch, bool hide)
{
    ...
    sprintf (buf, "the %s",
             title_table[ch->level][ch->sex ==
                                               SEX_MALE ? 0 : ch->sex == SEX_NEUTRAL? 2 : 1]);
    set_title (ch, buf);
...
void mobile_update (void)
{
    ...
    for (ch = char_list; ch != NULL; ch = ch_next)
    {
        ...
        if (ch->pIndexData->pShop != NULL)    /* give him some cash */
            if ( ch->cash < ch->pIndexData->wealth )
            {
                ch->cash +=
...
void char_update (void)
{
    ...
    for (ch = char_list; ch != NULL; ch = ch_next)
    {
		...
		if( is_affected(ch, gsn_earworm) )
			send_to_char("Laa la la la la la, laaaa la la la laaaaa\n\r", ch);
    ...
    for (obj = object_list; obj != NULL; obj = obj_next)
    {
        ...
        if (obj->carried_by != NULL)
        {
            if (IS_NPC (obj->carried_by)
                ...
                obj->carried_by->cash += obj->cost / 5;
            else
            {
