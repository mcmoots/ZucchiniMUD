...
void nanny (DESCRIPTOR_DATA * d, char *argument)
{
    ...
    switch (d->connected)
    {
        ...
        case CON_GET_NAME:
            ...
            if ( (parsename = check_parse_name(argument)) != NAME_GOOD)
            {
	        switch(parsename)
		{
		default:
                	...
                	return;
		case NAME_CLAN:
			send_to_desc( "That name is taken by a clan.\n\rName: ", d);
			break;
		case NAME_RESERVED:
			send_to_desc( "That name has been reserved.\n\rName: ", d);
			break;
		case NAME_SHORT:
			send_to_desc("Names must be at least three letters long.\n\rName: ", d);
			break;
		case NAME_LONG:
			send_to_desc("That name is too long.\n\rName: ", d);
			break;
		case NAME_NUMBER:
			send_to_desc("Names can contain only letters - no h4x0r crap!\n\rName: ", d);
			break;
		case NAME_MOB:
			send_to_desc("That's the name of a mob on this mud.\n\rName: ", d);
			break;
		case NAME_OBJ:
			send_to_desc("That's the name of an object on this mud.\n\rName: ", d);
			break;
		case NAME_COMMAND:
			send_to_desc("That name is already a command on this mud.\n\rName: ", d);
			break;
		case NAME_CAPS:
			send_to_desc("Check your capitalization.\n\rName: ", d);
			break;
		case NAME_TAKEN:
			send_to_desc("That name is already taken.\n\rName: ", d);
			break;
		}
		return;
            }
...
/* RT code for breaking link */
        ...
        case CON_GET_NEW_RACE:
            ...
            send_to_desc ("What is your sex (F/M/N)? ", d);
    ...
    case CON_READ_MOTD:
      if (ch->pcdata == NULL || ch->pcdata->pwd[0] == '\0')
	      ...
	      write_to_buffer (d, "Warning! Null password!\n\r", 0);
            ...
            write_to_buffer (d,
                             "\n\rWelcome to Zucchini. Please feed the monkeys.\n\r",
                             0);
            ch->next = char_list;
            char_list = ch;
		...
		if(mud_telnetga)
			SET_BIT (ch->comm, COMM_TELNET_GA);
                ...
                ch->train = 6;
                ch->practice = 5;
                sprintf (buf, "the %s", title_table[ch->level]
                         [ch->sex == SEX_FEMALE ? 1 : ch->sex == SEX_NEUTRAL ? 2 : 0]);
                set_title (ch, buf);

                do_function (ch, &do_outfit, "");
                obj_to_char (create_object (get_obj_index (OBJ_VNUM_MAP), 0), ch);
                obj_to_char (create_object (get_obj_index (OBJ_VNUM_GUIDE), 0), ch);
                obj_to_char (create_object (get_obj_index (OBJ_VNUM_NEWBIE_TREAT), 0), ch);
               
                /* One random weapon skill for free */
                int vnum = 0;
                
                i = number_range( 0, MAX_NEWBIE_CANDY - 1);
				vnum = newbie_candy_table[i];
				obj_to_char (create_object (get_obj_index (vnum), 0), ch);
