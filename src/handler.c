    ...
    {                            /* mobiles */
        ...
        if (skill_table[sn].spell_fun != spell_null)
            ...
            skill = 40 + ch->level;
                 ...
                 || sn == gsn_spear
                 || sn == gsn_mace
                 || sn == gsn_axe || sn == gsn_hand_to_hand
...
void affect_modify (CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd)
{
        ...
        (str_app[get_curr_stat (ch, STAT_STR)].wield * 10))
    {
        ...
        // WTF is this depth variable doing? We're not in a loop.
...
void obj_to_room (OBJ_DATA * obj, ROOM_INDEX_DATA * pRoomIndex)
{
    ...
    if( pRoomIndex->vnum == ROOM_VNUM_DONATE )
    	obj->cost = 0;
...
void deduct_cost (CHAR_DATA * ch, int cost)
{
    ...
    if (ch->cash < 0)
    {
        bugf ("deduct costs: cash %d < 0 after deducting %d", ch->cash, cost);
        ch->cash = 0;
    }
}
...
OBJ_DATA *create_money (int amount)
{
    ...
    if (amount == 1)
    {
        obj = create_object (get_obj_index (OBJ_VNUM_ZLOTY), 0);
    }

    else
    {
        obj = create_object (get_obj_index (OBJ_VNUM_CASH), 0);
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    if (room_is_dark (ch->in_room) && !IS_AFFECTED(ch, AFF_DARK_VISION) && !IS_AFFECTED (ch, AFF_INFRARED))
        return FALSE;
