...
/*
 * Snarf a mob section.  Zucchini style
 */
void load_mobiles (FILE * fp)
{
    ...
    for (;;)
    {
		...
		/* BRAINZ */
		pMobIndex->brain_content = str_dup (fread_word (fp));
...
/*
 * Snarf a mob section.  stock ROM.
 */
void load_stock_mobiles (FILE * fp)
{
    MOB_INDEX_DATA *pMobIndex;

    if (!area_last)
    {                            /* OLC */
        bug ("Load_stock_mobiles: no #AREA seen yet.", 0);
        exit (1);
    }
    ...
    for (;;)
    {
        ...
        if (letter != '#')
        {
            bug ("Load_stock_mobiles: # not found.", 0);
            exit (1);
        }
        ...
        if (get_mob_index (vnum) != NULL)
        {
            bug ("Load_stock_mobiles: vnum %d duplicated.", vnum);
            exit (1);
        }
        fBootDb = TRUE;
...
/*
 * Snarf an obj section. stock ROM.
 */
void load_stock_objects (FILE * fp)
{
    OBJ_INDEX_DATA *pObjIndex;
