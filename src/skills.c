...
void do_gain (CHAR_DATA * ch, char *argument)
{
    ...
    if (!str_prefix (arg, "list"))
    {
        ...
        for (sn = 0; sn < MAX_SKILL; sn++)
        {
            ...
            if (!ch->pcdata->learned[sn]
                && skill_table[sn].gain_cost > 0)
            {
                sprintf (buf, "%-18s %-5d ",
                         skill_table[sn].name,
                         skill_table[sn].gain_cost);
                send_to_char (buf, ch);
                if (++col % 3 == 0)
                    send_to_char ("\n\r", ch);
            }
    ...
    if (sn > 0)
    {
        ...
        {
            act ("$N tells you 'You already know that!'",
                 ch, NULL, trainer, TO_CHAR);
            return;
        }

        if (skill_table[sn].gain_cost <= 0)
        {
            act ("$N tells you 'I can't teach you that skill.'",
                 ch, NULL, trainer, TO_CHAR);
            return;
        }
	...
	{
		act("$N tells you 'That is beyond your powers.'", ch, NULL, trainer, TO_CHAR);
		return;
	}

        if (ch->cash < skill_table[sn].gain_cost)
        {
            act ("$N tells you 'You can't afford my lessons.'",
                 ch, NULL, trainer, TO_CHAR);
            return;
        }
        ...
        ch->cash -= skill_table[sn].gain_cost;
        return;
    }
...
void do_spells (CHAR_DATA * ch, char *argument)
{
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
        ...
        if ((level = skill_table[sn].skill_level) < LEVEL_HERO + 1
            && (fAll || level <= ch->level)
        ...
        {
            found = TRUE;
            level = skill_table[sn].skill_level;
            if (ch->level < level)
...
void list_group_costs (CHAR_DATA * ch)
{
    ...
    for (gn = 0; gn < MAX_GROUP; gn++)
    {
        ...
        if (!ch->gen_data->group_chosen[gn]
            ...
            && group_table[gn].rating > 0)
        {
            sprintf (buf, "%-18s %-5d ", group_table[gn].name,
                     group_table[gn].rating);
            send_to_char (buf, ch);
            if (++col % 3 == 0)
                send_to_char ("\n\r", ch);
        }
...
bool parse_gen_groups (CHAR_DATA * ch, char *argument)
{
    ...
    if (!str_prefix (arg, "add"))
    {
        ...
        if (gn != -1)
        {
            ...
            ch->gen_data->points_chosen += group_table[gn].rating;
            gn_add (ch, gn);
        ...
        if (sn != -1)
        {
            ...
            ch->gen_data->points_chosen += skill_table[sn].rating;
            ch->pcdata->learned[sn] = 1;
