...
bool spec_nasty (CHAR_DATA * ch)
{
    ...
    {
        case 0:
            act ("$n rips apart your coin purse, spilling your money!",
                 ch, NULL, victim, TO_VICT);
            act ("You slash apart $N's coin purse and gather his money.",
                 ch, NULL, victim, TO_CHAR);
            act ("$N's coin purse is ripped apart!",
                 ch, NULL, victim, TO_NOTVICT);
            amt = victim->cash / 10;    /* steal 10% of his cash */
            victim->cash -= amt;
            ch->cash += amt;
            return TRUE;
...
bool spec_thief (CHAR_DATA * ch)
{
    ...
    for (victim = ch->in_room->people; victim != NULL; victim = v_next)
    {
        ...
        {
            amount =
                victim->cash * UMIN (number_range (1, 20), ch->level / 2) / 100;
            amount = UMIN (amount, ch->level * ch->level * 25);
            ch->cash += amount;
            victim->cash -= amount;
            return TRUE;
        }
