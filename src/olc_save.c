...
void save_zmprogs (FILE * fp, AREA_DATA * pArea)
{
    MPROG_CODE *pMprog;
    int i;
...
/*****************************************************************************
 Name:        save_stock_mobile
 Purpose:     save one mobile, in stock format
 Called by:   save_stock_mobiles(olc_save.c).
 Notes:       Changed for ROM OLC.
 ****************************************************************************/
void save_stock_mobile (FILE * fp, MOB_INDEX_DATA * pMobIndex)
{
    sh_int race = pMobIndex->race;
    MPROG_LIST *pMprog;
    char buf[MAX_STRING_LENGTH];
    long temp;
    bool questmaster = FALSE, banker = FALSE, changer = FALSE;
    ...
    fprintf (fp, "%s~\n", fix_string (pMobIndex->description));
    fprintf (fp, "%s~\n", race_table[race].name);
    /* hack out new act flags */
    if( IS_SET(pMobIndex->act, ACT_QUESTMASTER))
    {
    	REMOVE_BIT(pMobIndex->act, ACT_QUESTMASTER);
	questmaster = TRUE;
    }
    if( IS_SET(pMobIndex->act, ACT_BANKER))
    {
    	REMOVE_BIT(pMobIndex->act, ACT_BANKER);
	banker = TRUE;
	if( !IS_SET(pMobIndex->act, ACT_IS_CHANGER));
	{
		SET_BIT(pMobIndex->act, ACT_IS_CHANGER);
		changer = TRUE;
	}
    }
    fprintf (fp, "%s ", fwrite_flag (pMobIndex->act, buf));
    
    /* hack new act flags back in */
    if( questmaster )
    	SET_BIT(pMobIndex->act, ACT_QUESTMASTER);
    if( changer )
    	REMOVE_BIT(pMobIndex->act, ACT_IS_CHANGER);
    if( banker )
    	SET_BIT(pMobIndex->act, ACT_BANKER);
    ...
    /* hack for new damage types */
    if( !str_cmp(attack_table[pMobIndex->dam_type].name, "karma") )
    	fprintf (fp, "drain\n");
    
    else if( !str_cmp(attack_table[pMobIndex->dam_type].name, "edrain") )
    	fprintf( fp, "magic\n");
	
    else if( !str_cmp(attack_table[pMobIndex->dam_type].name, "yodel") 
          || !str_cmp(attack_table[pMobIndex->dam_type].name, "yell")
	  || !str_cmp(attack_table[pMobIndex->dam_type].name, "scream") )
    	fprintf( fp, "none\n");
	
    else if( !str_cmp(attack_table[pMobIndex->dam_type].name, "razzle")
          || !str_cmp(attack_table[pMobIndex->dam_type].name, "dazzle")
	  || !str_cmp(attack_table[pMobIndex->dam_type].name, "laser")  )
	fprintf( fp, "none\n");
	
    else if( !str_cmp(attack_table[pMobIndex->dam_type].name, "psyblast")
          || !str_cmp(attack_table[pMobIndex->dam_type].name, "laugh")
          || !str_cmp(attack_table[pMobIndex->dam_type].name, "mdrain")  )
	fprintf( fp, "none\n");
...
/*****************************************************************************
 Name:        save_stock_mobiles
 Purpose:     Save #MOBILES secion of an area file in stock format.
 Called by:    save__stock_area(olc_save.c).
 Notes:         Changed for ROM OLC.
 ****************************************************************************/
void save_stock_mobiles (FILE * fp, AREA_DATA * pArea)
{
    int i;
    MOB_INDEX_DATA *pMob;
 ...
 */
	...
	case ITEM_BRAINS:
		fprintf (fp, "'%s' %d %d %d %d\n",
			 pObjIndex->value[0] != -1 ? skill_table[pObjIndex->value[0]].name : "",
			 pObjIndex->value[1], pObjIndex->value[2], pObjIndex->value[3], pObjIndex->value[4]);
		break;
    }
...
/*****************************************************************************
 Name:        save_stock_object
 Purpose:     Save one object to file in stock format.
 Called by:   save_stock_objects (below).
 ****************************************************************************/
void save_stock_object (FILE * fp, OBJ_INDEX_DATA * pObjIndex)
{
    char letter;
    AFFECT_DATA *pAf;
    EXTRA_DESCR_DATA *pEd;
    ...
    fprintf (fp, "%s~\n", fix_string (pObjIndex->description));
    fprintf (fp, "%s~\n", pObjIndex->material);
    /* hack to eliminate new item types */
    if( pObjIndex->item_type == ITEM_BRAINS )
    	fprintf( fp, "treasure " );
    else
	fprintf (fp, "%s ", item_name (pObjIndex->item_type));
    fprintf (fp, "%s ", fwrite_flag (pObjIndex->extra_flags, buf));
 ...
 */
	...
	/* write all zeros for new item types */
...
/*****************************************************************************
 Name:        save_stock_objects
 Purpose:     Save #OBJECTS section of an area file.
 Called by:   save_stock_area(olc_save.c).
 Notes:       Changed for ROM OLC.
 ****************************************************************************/
void save_stock_objects (FILE * fp, AREA_DATA * pArea)
{
    int i;
    OBJ_INDEX_DATA *pObj;
    ...
    for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
    {
             ...
             pRoomIndex = pRoomIndex->next)
        {
			...
			/* FUCK backwards compatibility! */
                        ...
                        fprintf (fp, "%s~\n", pExit->keyword);
			fprintf (fp, "%s ", fwrite_flag( pExit->rs_flags, buf )	);
...
/*****************************************************************************
 Name:        save_stock_rooms
 Purpose:    Save #ROOMS section of an area file.
 Called by:    save_stock_area(olc_save.c).
 ****************************************************************************/
void save_stock_rooms (FILE * fp, AREA_DATA * pArea)
{
    ROOM_INDEX_DATA *pRoomIndex;
    EXTRA_DESCR_DATA *pEd;
    EXIT_DATA *pExit;
    ...
    for (iHash = 0; iHash < MAX_KEY_HASH; iHash++)
    {
		...
		/* hack for new room flags */
		if IS_SET(pRoomIndex->room_flags, ROOM_GOOFY_EXITS);
		{
			goofy = TRUE;
			REMOVE_BIT(pRoomIndex->room_flags, ROOM_GOOFY_EXITS);
		}
		
                fprintf (fp, "%d ", pRoomIndex->room_flags);
		
		if ( goofy == TRUE )
			SET_BIT(pRoomIndex->room_flags, ROOM_GOOFY_EXITS);
...
/*****************************************************************************
 Name:        save_stock_area
 Purpose:     Save an area, stock rom format.
 Called by:   do_stocksave(olc_save.c).
 ****************************************************************************/
void save_stock_area (AREA_DATA * pArea)
{
    FILE *fp;
    char filename[MSL];
    
    strcpy(filename, pArea->file_name);
    strcat(filename, ".stock");

    logfile("save_stock_area: %s", filename); 

    fclose (fpReserve);
    if ( !(fp = fopen (filename, "w")) )
    {
        bug ("save_stock_area: fopen", 0);
        perror (filename);
    }
    ...
    save_stock_mobiles (fp, pArea);
    save_stock_objects (fp, pArea);
    save_stock_rooms (fp, pArea);
    save_specials (fp, pArea);
    save_resets (fp, pArea);
    save_shops (fp, pArea);
...
/*****************************************************************************
 Name:        do_stocksave
 Purpose:    Entry point for saving area data in stock ROM format.
 Called by:    interpreter(interp.c)
 ****************************************************************************/
void do_stocksave (CHAR_DATA * ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
    AREA_DATA *pArea;
    FILE *fp;
    ...
    if (arg1[0] == '\0')
    {
        ...
        {
            send_to_char ("Syntax:\n\r", ch);
            send_to_char ("  stocksave <vnum>   - saves a particular area\n\r", ch);
            send_to_char
                ("  stocksave area     - saves the area being edited\n\r", ch);
            send_to_char ("\n\r", ch);
        }
