 ...
 */
int check_parse_name args ((char *name));
...
int main (int argc, char **argv)
{
   ...
   int imcsocket = -1;
    ...
    boot_db ();
    log_string ("Zucchini is ready to rock.");
    game_loop_mac_msdos ();
#endif
    ...
    init_signals();  /* for the signal handler */

    boot_db ();
    logfile ("Zucchini is ready to rock on port %d (%s).", port, mud_ipaddress);
...
int init_socket (int port)
{
    ...
    logfile("Set IP address to %s", mud_ipaddress);
...
int check_parse_name (char *name)
{
    int clan;
    ...
    if (is_exact_name (name,
        "all auto immortal self someone something the you loner none unlinked"))
    {
     ...
     */
    {
        ...
        if (cleancaps
            || (total_caps > (strlen (name)) / 2
                && strlen (name) < 3)) return NAME_CAPS;
    }
              ...
              const void *arg2, int type, int min_pos)
{
    ...
    for (; to != NULL; to = to->next_in_room)
    {
        ...
        while (*str != '\0')
        {
		    ...
		    case 'g': 
		    	if (ch)
				i = IS_NPC(ch) ? "Mota" : god_table[ch->pcdata->god].name;
			else
				bug("Act: bad code $g for 'ch'", 0);
			break;
                    case 'd':
...
int colour (char type, CHAR_DATA * ch, char *string)
{
	...
	case 'u':
	    ...
	    break;
	case '$':
	    strcpy (code, C_BG_GREEN);
	    break;
	case '0':
	    strcpy (code, C_BG_BLACK);
	    break;
	case 'P':
	    strcpy (code, C_BG_MAGENTA);
	    break;
	case 'J':
	    strcpy (code, C_BG_YELLOW);
	    break;
	case 'V':
	    strcpy (code, C_BG_WHITE);
	    break;
	case 'T':
	    strcpy (code, C_BG_RED);
	    break;
	case 'U':
	    strcpy (code, C_BG_BLUE);
	    break;
	case 'v':
	    strcpy (code, C_BG_CYAN);
	    break;
        case '*':
            sprintf (code, "%c", '\a');
            break;
