...
void do_outfit (CHAR_DATA * ch, char *argument)
{
    ...
    {
        i = 0;
        vnum = OBJ_VNUM_SCHOOL_WEAPON;    /* just in case! */

	i = number_range( 0, MAX_NEWBIE_WEAPON - 1);

	vnum = newbie_weapon_table[i];
    ...
    act_new("You have been equipped by $g.", ch, NULL, NULL, TO_CHAR,
    POS_DEAD);
}
...
void do_award (CHAR_DATA *ch, char *argument)
{
	...
	if( arg1[0] == '\0' || arg2[0] == '\0' || !is_number(arg2) || arg3[0] == '\0')
	{
		...
		send_to_char("Valid types: bp qp cash\n\r", ch);
		return;
	}
	
	if((victim = get_char_world(ch, arg1)) == NULL)
	{
		send_to_char("They're not around.\n\r", ch);
		return;
	}
	...
	if( victim == ch )
	{
		send_to_char("No fair awarding yourself.\n\r", ch);
		return;
	}
	
	if( IS_IMMORTAL(victim))
	{
		send_to_char("Immortals don't get awards.\n\r", ch);
		return;
	}
	...
	if( !str_cmp(arg3, "bp"))
	{
		sprintf(buf, "%s gets an award of %d brownie point%s.\n\r", 
			victim->name, value, value == 1 ? "" : "s");
		send_to_char(buf, ch);
		
		victim->pcdata->browniepoints += value;
		
		sprintf(buf, "%s has given you %d brownie point%s.\n\r",
			ch->name, value, value == 1 ? "" : "s");
		send_to_char(buf, victim);
		
		sprintf(buf, "$N awards %d brownie point%s to %s.",
			value, value == 1 ? "" : "s", victim->name);
		wiznet(buf, ch, NULL, WIZ_PENALTIES, WIZ_SECURE, 0);
	...
	if( !str_cmp(arg3, "qp"))
	{
		sprintf(buf, "%s gets an award of %d quest point%s.\n\r", 
			victim->name, value, value == 1 ? "" : "s");
		send_to_char(buf, ch);
		
		victim->pcdata->quest_curr += value;
		
		sprintf(buf, "%s has given you %d quest point%s.\n\r",
			ch->name, value, value == 1 ? "" : "s");
		send_to_char(buf, victim);
		
		sprintf(buf, "$N awards %d quest point%s to %s.",
			value, value == 1 ? "" : "s", victim->name);
		wiznet(buf, ch, NULL, WIZ_PENALTIES, WIZ_SECURE, 0);
	...
	if( !str_cmp(arg3, "cash"))
	{
		sprintf(buf, "%s gets an award of %d zloty%s.\n\r", 
			victim->name, value, value == 1 ? "" : "s");
		send_to_char(buf, ch);
		
		victim->cash += value;
		
		sprintf(buf, "%s has given you %d zloty%s.\n\r",
			ch->name, value, value == 1 ? "" : "s");
		send_to_char(buf, victim);
		
		sprintf(buf, "$N awards %d zloty%s to %s.",
			value, value == 1 ? "" : "s", victim->name);
		wiznet(buf, ch, NULL, WIZ_PENALTIES, WIZ_SECURE, 0);
	...
	send_to_char("Syntax: award <char> <amount> <type>\n\r", ch);
	send_to_char("Where type is one of: bp qp cash\n\r", ch);
	return;
}
...
void do_mstat (CHAR_DATA * ch, char *argument)
{
    ...
    sprintf (buf,
             "Lv: %d  Align: %d  Cash: %ld  Exp: %d\n\r",
             victim->level,
             victim->alignment, victim->cash, victim->exp);
    send_to_char (buf, ch);
...
void do_protect( CHAR_DATA *ch, char *argument)
{
	...
	if( IS_SET(victim->act, PLR_SAFE))
	{
		REMOVE_BIT(victim->act, PLR_SAFE);
		send_to_char("They are no longer protected.\n\r", ch);
		send_to_char("You feel vulnerable.\n\r", victim);
	}
	
	else
	{
		SET_BIT(victim->act, PLR_SAFE);
		send_to_char("They are now protected from aggressive mobs.\n\r", ch);
		send_to_char("You feel unafraid.\n\r", victim);
	}
	
}

void do_snoopproof (CHAR_DATA * ch, char *argument)
{
    CHAR_DATA *victim;

    if (argument[0] == '\0')
    {
        send_to_char ("Snoop-proof whom?\n\r", ch);
        return;
    }
...
void do_mset (CHAR_DATA * ch, char *argument)
{
    ...
    if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
    {
        ...
        send_to_char ("    str int wis dex con sex level god\n\r", ch);
        send_to_char ("    race group cash hp mana move prac\n\r", ch);
        send_to_char ("    align train thirst hunger drunk full\n\r", ch);
        send_to_char ("    balance security hours\n\r", ch);
        return;
    }
	...
	if( IS_NPC(victim))
	{
    	...
    	if( !is_number(arg3))
	{
		send_to_char("Balance should be a number.\n\r", ch);
		return;
	}
	
	if( value < 0 )
	{
		send_to_char("You can't overdraft on someone else's bank account!\n\r", ch);
		return;
	}
	...
	if( IS_NPC(victim))
	{
		send_to_char("All NPCs worship Mota.\n\r", ch);
		return;
	}
	if( god == 0 )
	{
...
void do_copyover (CHAR_DATA * ch, char *argument)
{
   ...
   strncpy( buf4, "-1", 100 );
    ...
    execl (EXE_FILE, "rom", buf, "copyover", buf2, buf3, buf4, (char *) NULL);
...
void qmconfig_read (void) {
	...
	logfile("Loading configuration settings from ../area/qmconfig.rc.");

	fp = fopen("../area/qmconfig.rc","r");
	if (!fp) {
		logfile("qmconfig.rc not found. Using compiled-in defaults.");
		return;
	}
	...
	for(;;) {
		...
		if (!fMatch) {
			logfile("qmconfig_read: no match for %s!", word);
			fread_to_eol(fp);
		}
	}	
	logfile("Settings have been read from ../area/qmconfig.rc");
	exit(0);
...
void do_arealinks(CHAR_DATA *ch, char *argument)
{
    ...
    fclose(fpReserve);
    if( (fp = fopen("arealinks.txt", "w")) == NULL)
    {
	...
	if ( parea == NULL ) /* just in case */
	{
		bugf("arealinks: null area!");
		return;
	}
	...
	if (!found)
	{
                sprintf(buf, "    No links to other areas found.\n\r");
		fprintf(fp, buf);
	}
    }
...
void do_brainlist(CHAR_DATA *ch, char *argument)
{
	FILE *fp;
	AREA_DATA *pArea;
	MOB_INDEX_DATA *pMob;
	...
	fclose(fpReserve);
	if( (fp = fopen("brainlist.txt", "w")) == NULL)
	{
		send_to_char("Error opening file, stopping.\n\r",ch);
		fclose(fp);
		fpReserve = fopen(NULL_FILE, "r");
		return;
	}
...
void do_helpcheck (CHAR_DATA *ch, char *argument)
{
    ...
    if ( argument[0] == '\0' )
    {
        send_to_char( "Syntax: helpcheck <commands/skills>\n\r", ch );
        return;
    }
    
    if ( !str_cmp ( argument, "commands" ) )
    {
        send_to_char ( "Checking for missing command help entries....\n\r", ch );
        for ( i = 0; cmd_table[i].name[0] != '\0'; i++ )
        {
            ...
            if ( !found )
            {
                sprintf ( buf, "%-20s", cmd_table[i].name );
                send_to_char( buf, ch );
                if ( ++col % 4 == 0 )
                    send_to_char ( "\n\r", ch );
            }
    ...
    {
        send_to_char ( "Checking for missing skill help entries....\n\r", ch );
        for ( i = 0; i < MAX_SKILL; i++ )
        {
		logfile("Skill %d, %s", i, skill_table[i].name);
            found = FALSE;
            for ( pHelp = help_first; pHelp != NULL; pHelp = pHelp->next )
            {
                if ( pHelp->keyword[0] == '\0' )
...
void do_immtitle(CHAR_DATA *ch, char *argument)
{
	if( IS_NPC(ch))
	{
		send_to_char("You're an NPC, dude.\n\r", ch);
		return;
	}
  ...
  if (colorstrlen(argument) > 8) 
  {
    send_to_char("Immtitle must be 8 characters or less.\n\r", ch); 
    return;
  }
