...
/*
 * Allows the mobile teach a character a new skill/spell
 * Syntax: mob teach [victim] [skill/spell]
 */
void do_mpteach( CHAR_DATA *ch, char *argument)
{
	...
	{
		bugf("MpTeach: Invalid skill %s.", arg);
		return;
	}
	
	else if( IS_NPC(victim) )
	{
		bugf("MpTeach: tried to teach %s, an NPC.", victim->name);
		return;
	}
	
	else if( victim->pcdata->learned[sn] > 1 )
		return;
	...
	else
	{
		...
		sprintf(buf, "You have learned the art of %s.", skill_table[sn].name);
