...
void load_stock_mobiles args ((FILE * fp));
void load_old_obj args ((FILE * fp));
void load_objects args ((FILE * fp));
void load_stock_objects args ((FILE * fp));
void load_resets args ((FILE * fp));
void load_rooms args ((FILE * fp));
void load_stock_rooms args ((FILE * fp));
void load_shops args ((FILE * fp));
void load_socials args ((FILE * fp));
void load_specials args ((FILE * fp));
void load_bans args ((void));
void load_mobprogs args ((FILE * fp));
void load_zmprogs args ((FILE * fp));
...
void boot_db ()
{
     ...
     */
    {
        ...
        {
		logfile("problem opening area list!");
            perror (AREA_LIST);
            exit (1);
        }
	logfile("area list loaded");
        ...
        for (;;)
        {
            ...
            for (;;)
            {
                ...
                if (word[0] == '$')
                    ...
                    load_area (fpArea);
		    logfile("loading %d", current_area);
				}
                /* OLC */
                    ...
                    load_stock_mobiles (fpArea);
				else if (!str_cmp (word, "ZMOBILES"))
					load_mobiles (fpArea);
                else if (!str_cmp (word, "MOBPROGS"))
                    load_mobprogs (fpArea);
                else if (!str_cmp (word, "ZMPROGS"))
                	load_zmprogs (fpArea);
                else if (!str_cmp (word, "OBJOLD"))
                    load_old_obj (fpArea);
                else if (!str_cmp (word, "ZOBJECTS"))
                    load_objects (fpArea);
				else if (!str_cmp (word, "OBJECTS"))
					load_stock_objects (fpArea);
                else if (!str_cmp (word, "RESETS"))
                    load_resets (fpArea);
                else if (!str_cmp (word, "ROOMS"))
                    load_stock_rooms (fpArea);
				else if (!str_cmp (word, "ZROOMS"))
					load_rooms (fpArea);
...
void load_area (FILE * fp)
{
	...
	logfile("Loading area %s", pArea->file_name);
...
void load_old_mob (FILE * fp)
{
    ...
    for (;;)
    {
        ...
        if (letter != 'S')
        {
            bug ("Load_old_mob: vnum %d non-S.", vnum);
            exit (1);
        }
...
/*
 * Snarf a room section, stock format.
 */
void load_stock_rooms (FILE * fp)
{
    ROOM_INDEX_DATA *pRoomIndex;
             ...
             pRoomIndex != NULL; pRoomIndex = pRoomIndex->next)
        {
            for (door = 0; door <= 5; door++)
            {
                if ((pexit = pRoomIndex->exit[door]) != NULL
                    ...
                    && !IS_SET(pRoomIndex->room_flags, ROOM_GOOFY_EXITS))
                {
...
void load_mobprogs (FILE * fp)
{
	...
	logfile("Loaded %d mobprogs.", top_mprog_index);
    return;
}

/*
 * Load mobprogs section - Zucchini format
 */
void load_zmprogs (FILE * fp)
{
    MPROG_CODE *pMprog;
                        ...
                        switch (pObjIndex->item_type)
                        {
                            ...
                            case ITEM_SCROLL:
                                ...
                                for (i = 1; i < 5; i++)
                                {
                                    ...
                                    {
                                      olevel = UMIN (olevel, skill_table[pObjIndex->value[i]].skill_level);
                                    }
                                }
...
CHAR_DATA *create_mobile (MOB_INDEX_DATA * pMobIndex)
{
    ...
    mob->brain_content = str_dup (pMobIndex->brain_content);
    mob->id = get_mob_id ();
    ...
    {
        mob->cash = number_range (pMobIndex->wealth / 2, 3 * pMobIndex->wealth / 2);
    }
...
void clone_mobile (CHAR_DATA * parent, CHAR_DATA * clone)
{
    ...
    clone->brain_content = str_dup (parent->brain_content);
...
void do_areas (CHAR_DATA * ch, char *argument)
{
	...
	{
		send_to_char( "Syntax: areas [#level]\n\r", ch);
		return;
	}
	...
	while( pArea1 != NULL )
	{		
		while( pArea1->low_range > ilevel || pArea1->high_range < ilevel )
		{
			pArea1 = pArea1->next;
			if( pArea1 == NULL )
				break;
		...
		if( pArea2 != NULL )
		{		
			while( pArea2->low_range > ilevel || pArea2->high_range < ilevel )
			{
				pArea2 = pArea2->next;
