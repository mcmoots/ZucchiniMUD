...
const char *i3ansi_conversion[I3MAX_ANSI][3] =
{
	...
	{ "{D", "%^BLACK%^",   "\e[0;0;30m" }, // Black
	{ "{r", "%^RED%^",     "\e[0;0;31m" }, // Dark Red
	{ "{g", "%^GREEN%^",   "\e[0;0;32m" }, // Dark Green
	{ "{y", "%^ORANGE%^",  "\e[0;0;33m" }, // Orange/Brown
	{ "{b", "%^BLUE%^",    "\e[0;0;34m" }, // Dark Blue
	{ "{m", "%^MAGENTA%^", "\e[0;0;35m" }, // Purple/Magenta
	{ "{c", "%^CYAN%^",    "\e[0;0;36m" }, // Cyan
	{ "{w", "%^WHITE%^",   "\e[0;0;37m" }, // Grey
	/* Background colors */
	{ "{O", "%^B_BLACK%^",   "\e[40m" }, // Black
	{ "{T", "%^B_RED%^",     "\e[41m" }, // Red
	{ "{$", "%^B_GREEN%^",   "\e[42m" }, // Green
	{ "{J", "%^B_ORANGE%^",  "\e[43m" }, // Orange
	{ "{J", "%^B_YELLOW%^",  "\e[43m" }, // Yellow, which may as well be orange since ANSI doesn't do that
	{ "{U", "%^B_BLUE%^",    "\e[44m" }, // Blue
	{ "{P", "%^B_MAGENTA%^", "\e[45m" }, // Purple/Magenta
	{ "{v", "%^B_CYAN%^",    "\e[46m" }, // Cyan
	{ "{V", "%^B_WHITE%^",   "\e[47m" }, // White
	/* Text Affects */
	{ "{x", "%^RESET%^",     "\e[0m" }, // Reset Text
	{ "{x", "%^RESET%^",     "\e[0m" }, // Reset Text
	{ "{x", "%^BOLD%^",      "\e[1m" }, // Bolden Text(Brightens it)
	{ "{x", "%^EBOLD%^",	 "\e[0m" }, // Assumed to be a reset tag to stop bold
	{ "{x", "%^UNDERLINE%^", "\e[4m" }, // Underline Text
	{ "{x", "%^FLASH%^",     "\e[5m" }, // Blink Text
	{ "{x", "%^ITALIC%^",    "\e[6m" }, // Italic Text
	{ "{u", "%^REVERSE%^",   "\e[7m" }, // Reverse Background and Foreground Colors
	/* Foreground extended colors */
	{ "{D", "%^BLACK%^%^BOLD%^",   "\e[0;1;30m" }, // Dark Grey
	{ "{R", "%^RED%^%^BOLD%^",     "\e[0;1;31m" }, // Red
	{ "{G", "%^GREEN%^%^BOLD%^",   "\e[0;1;32m" }, // Green
	{ "{Y", "%^YELLOW%^",          "\e[0;1;33m" }, // Yellow
	{ "{B", "%^BLUE%^%^BOLD%^",    "\e[0;1;34m" }, // Blue
	{ "{M", "%^MAGENTA%^%^BOLD%^", "\e[0;1;35m" }, // Pink
	...
	{ "{x", "%^RED%^%^FLASH%^",             "\e[0;5;31m" }, // Dark Red
	{ "{x", "%^GREEN%^%^FLASH%^",           "\e[0;5;32m" }, // Dark Green
	{ "{x", "%^ORANGE%^%^FLASH%^",          "\e[0;5;33m" }, // Orange/Brown
	{ "{x", "%^BLUE%^%^FLASH%^",            "\e[0;5;34m" }, // Dark Blue
	{ "{x", "%^MAGENTA%^%^FLASH%^",         "\e[0;5;35m" }, // Magenta/Purple
	{ "{x", "%^CYAN%^%^FLASH%^",            "\e[0;5;36m" }, // Cyan
	{ "{x", "%^WHITE%^%^FLASH%^",           "\e[0;5;37m" }, // Grey
	{ "{x", "%^BLACK%^%^BOLD%^%^FLASH%^",   "\e[1;5;30m" }, // Dark Grey
	{ "{x", "%^RED%^%^BOLD%^%^FLASH%^",     "\e[1;5;31m" }, // Red
	{ "{x", "%^GREEN%^%^BOLD%^%^FLASH%^",   "\e[1;5;32m" }, // Green
	{ "{x", "%^YELLOW%^%^FLASH%^",          "\e[1;5;33m" }, // Yellow
	{ "{x", "%^BLUE%^%^BOLD%^%^FLASH%^",    "\e[1;5;34m" }, // Blue
	{ "{x", "%^MAGENTA%^%^BOLD%^%^FLASH%^", "\e[1;5;35m" }, // Pink
	{ "{x", "%^CYAN%^%^BOLD%^%^FLASH%^",    "\e[1;5;36m" }, // Light Blue
	{ "{x", "%^WHITE%^%^BOLD%^%^FLASH%^",   "\e[1;5;37m" }  // White
};
...
/* Some more code borrowed from the IMC code to handle the channel list */
 ...
 * and IMC used as well. Figured it would do the job nicely here too.
...
/* Thanks to the IMC folks - this function is a modified form of verify_format from the IMC code.
...
void I3_connection_close( bool reconnect ) 
{
   ...
   if( reconnect )
   {
      if( reconattempts <= 5 )
	{
	   ...
	   i3log( "Will attempt to reconnect in approximately 5 seconds." );
      }
	else if( reconattempts <= 20 )
	{
	   i3wait = 5000; /* Wait for 5000 game loops */
         i3log( "Will attempt to reconnect in approximately 5 minutes due to extended failures." );
	}
	else
	{
...
bool I3_write_packet( char *msg ) 
{
   ...
   {
	if( check < 0 )
	   i3log( "Write error on socket." );
	else
	   i3log( "EOF encountered on socket write." );
...
int I3_process_startup_reply( char *s ) 
{
   ...
   reconattempts = 0;
   i3wait = 0;
   i3log( "Intermud-3 Network connection complete." );
   return 0;
}
...
void update_chanhistory( I3_CHANNEL *channel, char *message )
{
   ...
   if( !channel )
   {
	i3bug( "update_chanhistory: NULL channel received!" );
	return;
   }
   if( !message || message[0] == '\0' )
   {
	i3bug( "update_chanhistory: NULL message received!" );
	return;
   }
   strcpy( msg, message );
...
int I3_process_channel_t( char *s )
{
   ...
   for( d = first_descriptor; d; d = d->next ) 
   {
	...
	if( !I3_hasname( I3LISTEN(d->character), channel->local_name ) )
	   continue;
	sprintf( lname, "%s@%s", CH_NAME(d->character), I3_THISMUD );
...
void I3_beep( CHAR_DATA *ch, char *argument )
{
   ...
   {
	i3_to_char( "You should specify a person and a mud.\n\r"
	    "(use {Gi3 mudlist{x to get an overview of the muds available)\n\r", ch);
	return;
   }
   ps[0] = 0;
   ps++;
...
int I3_process_tell( char *s ) 
{
   ...
   I3_get_field( ps, &next_ps );
   I3_remove_quotes( &ps );
   i3_printf( ch, "%s@%s tells you: ", ps, header.originator_mudname );
...
int I3_process_who_req( char *s ) 
{
   ...
   {
      outbuf[0] = '\0';
      sprintf( buf, "{WPlayers on {G%s {g[telnet://%s]{x", I3_THISMUD, this_mud->telnet);           
      strcat( outbuf, buf );
   ...
   {
      I3_write_buffer( "({\"" );
      strcpy( headerbuf, "{g*** {GMortals{g ***{x" );
   ...
   {
      I3_write_buffer( "({\"" );
      strcpy( headerbuf, "{g*** {GImmortals{g ***{x" );
      send_to_i3( I3_escape( headerbuf ) );
      I3_write_buffer( "\"," );
	if( xx > 0 )
   ...
   I3_write_buffer( "({\"" );
   sprintf( tailbuf, "{g[{G%d Player%s{g]", pcount, pcount == 1 ? "" : "s" );
   send_to_i3( I3_escape( tailbuf ) );
   I3_write_buffer( "\"," );
   sprintf( smallbuf, "%ld", bogusidle * 2 );
   I3_write_buffer( smallbuf );
   I3_write_buffer( ",\"" );
   sprintf( tailbuf, "{g[{GHomepage: %s{g] [{G%3d Max Since Reboot{g]{x", this_mud->web, I3MAXPLAYERS );
   send_to_i3( I3_escape( tailbuf ) );
   I3_write_buffer( "\",}),}),})\r" );
   I3_send_packet( );
   return 0;
...
int I3_process_finger_reply( char *s ) 
{
    ...
    I3_get_field( ps, &next_ps );
    I3_remove_quotes( &ps );
    i3_printf( ch, "{WI3FINGER information for {G%s{g@{G%s{x\n\r", ps, header.originator_mudname );
    i3_to_char( "-------------------------------------------------\n\r", ch );
    ps = next_ps;
    I3_get_field( ps, &next_ps );
    I3_remove_quotes( &ps );
...
int I3_process_locate_reply( char *s ) 
{
   ...
   i3_printf( ch, "{RI3 Locate: {Y%s{y@{Y%s: {c%s{x\n\r", user_name, mud_name, status );
   return 0;
}
...
char *i3fread_string( FILE *fp )
{
	...
	if ( feof(fp) )
	{
	    i3bug( "i3fread_string: EOF encountered on read.\n\r" );
	    return I3STRALLOC( "" );
	}
	c = getc( fp );
    }
	...
	if ( ln >= (MSL - 1) )
	{
	     i3bug( "i3fread_string: string too long" );
	     *plast = '\0';
...
int i3fread_number( FILE *fp )
{
    ...
    do
    {
        if ( feof(fp) )
        {
          i3log( "i3fread_number: EOF encountered on read." );
          return 0;
        }
	c = getc( fp );
    }
...
void I3_load_ucache( void )
{
    FILE *fp;
    UCACHE_DATA *user;
    i3log( "Loading ucache data..." );
    if( ( fp = fopen( I3_UCACHE_FILE, "r" ) ) == NULL ) 
    {
	i3log( "No ucache data found." );
	return;
    }
    for ( ; ; )
    {
...
bool I3_read_config( int mudport ) 
{
   ...
   this_mud->adminlevel  = I3MAX_LEVEL; /* Default for administration level */
   ...
   {
	i3log( "Mud name not loaded in configuration file." );
	i3log( "Network configuration aborted." );
...
void I3_loadchannels( void )
{
   ...
   {
	i3log( "No channel config file found." );
	return;
   }
   for ( ; ; )
   {
...
void I3_write_channel_config( void ) 
{
    ...
    {
	i3log( "Couldn't write to channel config file." );
	return;
    }
    for( channel = first_I3chan; channel; channel = channel->next ) 
    {
...
void I3_chanlist( CHAR_DATA *ch, char *argument ) 
{
   ...
   for( channel = first_I3chan; channel; channel = channel->next ) 
   {
	...
	if( channel->local_name && I3_hasname( I3LISTEN(ch), channel->local_name ) )	
	   found = TRUE;
	sprintf( buf, "{C%c {W%-18s{Y%-4d{B%-20s{W%-20s%-8s\n\r",
	    found ? '*' : ' ',
   ...
   strcpy( buf, "{C*: You are listening to these channels.{x\n\r" );
   i3send_to_pager( buf, ch );
   return;
}
...
void I3_setup_channel( CHAR_DATA *ch, char *argument ) 
{
   ...
   if( CH_LEVEL(ch) < this_mud->adminlevel ) 
   {
	i3_to_char( "{RThat option is restricted to mud administrators only.{x\n\r", ch );
	return;
   }
   if( !I3_is_connected() )
   {
   ...
   {
	i3_to_char("Unknown channel\n\r"
	    "(use {Wi3 chanlist{xto get an overview of the channels available)\n\r", ch );
	return;
   }
   if( localname[0] == '\0' )
   {
...
void I3_edit_channel( CHAR_DATA *ch, char *argument ) 
{
   ...
   {
	i3_to_char("Unknown local channel\n\r"
...
void I3_chan_who( CHAR_DATA *ch, char *argument ) 
{
   ...
   if( !I3_is_connected() )
   {
    ...
    {
	i3_to_char( "Unknown channel.\n\r"
	    "(use {Gi3 chanlist{x to get an overview of the channels available)\n\r", ch );
	return;
    }
    if( ( mud = find_I3_mud_by_name( argument ) ) == NULL ) 
    {
	i3_to_char( "Unknown mud.\n\r"
	    "(use {Gi3 mudlist{x to get an overview of the muds available)\n\r", ch );
	return;
    }
    if( mud->status >= 0 ) 
...
void I3_mudinfo( CHAR_DATA *ch, char *argument ) 
{
   ...
   if( !argument || argument[0] == '\0' ) 
   {
	i3_to_char( "Which mud do you want information about?\n\r"
	    "(use {Gi3 mudlist{x to get an overview of the muds available)\n\r", ch );
	return;
   }
   if( ( mud = find_I3_mud_by_name( argument ) ) == NULL ) 
...
void I3_send_user_req( CHAR_DATA *ch, char *argument )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
	i3_to_char( "Query who at which mud?\n\r"
	    "(use {gi3 mudlist{x to get an overview of the muds available)\n\r", ch );
	return;
   }
...
/*
   The filtered packet is returned to the router in the following packet : 
                         ({
                             ...
                             (int)      5,
                             (string)   originator_mudname,    // The chan owner mudname
                             (string)   0,
                             (string)   target_mudname,        // the router
                             (string)   0,
                             (string)   channel_name,
...
/* To administer a channel, the channels host/owner mud may use the following packet: 
    ...
    The muds specified in add_to_list are added to the allowed/banned list stored
    within the router network. The muds specified in remove_from_list are removed 
    from that list. 
*/
/*
({"channel-admin",5,"mudname","person","*gjs",0,"I3_channelname",({"Mudtoadd",}),({}),})
({"channel-admin",5,"mudname","person","*gjs",0,"I3_channelname",({}),({"Mudtoremove",}),})
*/
int I3_send_channel_admin( CHAR_DATA *ch, char *chan_name, char *list )
{
   if( !I3_is_connected() )
	return 0;
   I3_write_header( "channel-admin", I3_THISMUD, ch->name, I3_ROUTER_NAME, NULL );
   I3_write_buffer( "\"" );
...
*/
/*
({"channel-add",5,"Alsherok_Code","Samson","*gjs",0,"arthmoor",1,})
*/
int I3_send_channel_add( CHAR_DATA *ch, char *arg, int type )
{
   if( !I3_is_connected() )
	return 0;
   I3_write_header( "channel-add", I3_THISMUD, ch->name, I3_ROUTER_NAME, NULL );
   I3_write_buffer( "\"" );
   I3_write_buffer( arg );
   I3_write_buffer( "\"," );
   switch( type )
   {
	default:
	   i3bug( "I3_send_channel_add: Illegal channel type!" );
	   return 0;
	case 0:
	   I3_write_buffer( "0,})\r" );
	   break;
...
void I3_addchan( CHAR_DATA *ch, char *argument )
{
   ...
   {
	i3_printf( ch, "%s is already hosted by %s.\n\r", channel->I3_name, channel->host_mud );
	return;
   }
   if( ( channel = find_I3_channel_by_localname( arg2 ) ) != NULL )
   {
	i3_printf( ch, "Channel %s@%s is already locally configured as %s.\n\r",
	   channel->I3_name, channel->host_mud, channel->local_name );
	return;
   }
   if( !isdigit( argument[0] ) )
   {
	i3_to_char( "Invalid type. Must be numerical.\n\r", ch );
	I3_addchan( ch, "" );
	return;
   }
   type = atoi( argument );
   if( type < 0 || type > 2 )
   {
	i3_to_char( "Invalid channel type.\n\r", ch );
	I3_addchan( ch, "" );
	return;
   }
   i3_printf( ch, "Adding channel to router: {W%s{x\n\r", arg );
   ...
   channel->layout_m = I3STRALLOC( "{R[{W%s{R] {C%s@%s: {c%s{x" );
   channel->layout_e = I3STRALLOC( "{R[{W%s{R] {c%s{x" );
   for( x = 0; x < 20; x++ )
	channel->history[x] = NULL;
   I3LINK( channel, first_I3chan, last_I3chan, next, prev );
   if( type != 0 )
   {
      ...
      I3_send_channel_admin( ch, channel->I3_name, buf );
      i3_to_char( "{GChannel mudlist updated.{x\n\r", ch );
   }
   i3_printf( ch, "{Y%s{y@{Y%s {xis now locally known as {Y%s\n\r", channel->I3_name, channel->host_mud, channel->local_name );
   I3_send_channel_listen( channel, TRUE );
   I3_write_channel_config();
   return;
}
...
int I3_send_channel_remove( CHAR_DATA *ch, I3_CHANNEL *channel )
{
   if( !I3_is_connected() )
	return 0;
   I3_write_header( "channel-remove", I3_THISMUD, ch->name, I3_ROUTER_NAME, NULL );
   I3_write_buffer( "\"" );
   I3_write_buffer( channel->I3_name );
   I3_write_buffer( "\",})\r" );
   I3_send_packet( );
...
void I3_removechan( CHAR_DATA *ch, char *argument )
{
   ...
   if( !argument || argument[0] == '\0' )
   {
	i3_to_char( "Usage: i3 chanremove <channel>\n\r", ch );
	i3_to_char( "Channelname should be the name seen on 'chanlist all'\n\r", ch );
	return;
   }
   if( ( channel = find_I3_channel_by_name( argument ) ) == NULL )
   {
	i3_to_char( "No channel by that name exists.\n\r", ch );
	return;
   }
   if( str_cmp( channel->host_mud, I3_THISMUD ) )
   {
	i3_printf( ch, "%s does not host this channel and cannot remove it.\n\r", I3_THISMUD );
	return;
   }
   i3_printf( ch, "Removing channel from router: %s\n\r", channel->I3_name );
   I3_send_channel_remove( ch, channel );
   i3_printf( ch, "Destroying local channel entry for %s\n\r", channel->I3_name );
   destroy_I3_channel( channel );
   I3_write_channel_config();
   return;
}
...
void I3_setconfig( CHAR_DATA *ch, char *argument )
{
   ...
   if( !arg || arg[0] == '\0' )
   {
	i3_to_char( "Configuration info for your mud. Changes save when edited.\n\r", ch );
	i3_to_char( "You can set the following:\n\r\n\r", ch );
	i3_to_char( "Show       : Displays your current congfiguration.\n\r", ch );
	i3_to_char( "Autoconnect: A toggle. Either on or off. Your mud will connect automatically with it on.\n\r", ch );
	i3_to_char( "Mudname    : The name you want displayed on I3 for your mud.\n\r", ch );
	i3_to_char( "Telnet     : The telnet address for your mud. Do not include the port number.\n\r", ch );
	i3_to_char( "Web        : The website address for your mud. In the form of: www.address.com\n\r", ch );
	i3_to_char( "Admin      : The email address of your mud's administrator. Needs to be valid!!\n\r", ch );
	i3_to_char( "Status     : The open status of your mud. IE: Public, Development, etc.\n\r", ch );
	i3_to_char( "Mudtype    : What type of mud you have. Diku, Rom, Smaug, Merc, etc.\n\r", ch );
	i3_to_char( "Baselib    : The base version of the codebase you have.\n\r", ch );
	i3_to_char( "Mudlib     : Whatever you call your mud's current codebase.\n\r", ch );
	i3_to_char( "Minlevel   : Minimum level at which I3 will recognize your players.\n\r", ch );
	i3_to_char( "Adminlevel : The level at which administrative commands become available.\n\r", ch );
	return;
   }
   if( !strcmp( arg, "show" ) )
   {
	i3_printf( ch, "Mudname       : %s\n\r", this_mud->name );
	i3_printf( ch, "Autoconnect   : %s\n\r", this_mud->autoconnect == TRUE ? "Enabled" : "Disabled" );
	i3_printf( ch, "Telnet address: %s:%d\n\r", this_mud->telnet, this_mud->player_port );
	i3_printf( ch, "Website       : %s\n\r", this_mud->web );
	i3_printf( ch, "Admin Email   : %s\n\r", this_mud->admin_email );
	i3_printf( ch, "Status        : %s\n\r", this_mud->open_status );
	i3_printf( ch, "Mudtype       : %s\n\r", this_mud->mud_type );
	i3_printf( ch, "Base Mudlib   : %s\n\r", this_mud->base_mudlib );
	i3_printf( ch, "Mudlib        : %s\n\r", this_mud->mudlib );
	i3_printf( ch, "Minlevel      : %d\n\r", this_mud->minlevel );
	i3_printf( ch, "Adminlevel    : %d\n\r", this_mud->adminlevel );
	return;
   }
...
void I3_other( CHAR_DATA *ch, char *argument )
{
	...
	"ignore", "invis", "packetdebug", "user", "config", "ucache", "chanedit", NULL ) ) 
   {
	...
	default:
	   i3_to_char( "General Usage:\n\r", ch );
         i3_to_char( "------------------------------------------------\n\r\n\r", ch );
	   i3_to_char( "List channels available                : i3 chanlist [all] [filter]\n\r", ch );
	   i3_to_char( "To tune into a channel                 : i3 listen <localchannel>\n\r", ch );
	   i3_to_char( "To see who is listening on another mud : i3 chanwho <channel> <mud>\n\r", ch );
	   i3_to_char( "List muds connected to I3              : i3 mudlist [filter]\n\r", ch );
	   i3_to_char( "Information on antoher mud             : i3 mudinfo <mud>\n\r", ch );
	   i3_to_char( "Ignore someone who annoys you          : i3ignore <person@mud>\n\r", ch );
	   i3_to_char( "Make yourself invisible to I3          : i3 invis\n\r", ch );
         if( CH_LEVEL(ch) >= this_mud->adminlevel )
         {
		i3_to_char( "\n\rAdministration functions\n\r", ch );
		i3_to_char( "------------------------------------------------\n\r\n\r", ch );
		i3_to_char( "{GGeneral statistics:{x\n\r", ch );
	      i3_to_char( "i3 stats\n\r", ch );
		i3_to_char( "i3 ucache\n\r", ch );
		i3_to_char( "i3 user <person@mud>\n\r\n\r", ch );
		i3_to_char( "Local channel setup and editing:\n\r", ch );
	      i3_to_char( "i3 setup <i3channelname> <localname> [level]\n\r", ch );
		i3_to_char( "i3 chanedit <localchannel>\n\r", ch );
	      i3_to_char( "i3 chanlayout <localchannel> <message> <emote>\n\r\n\r", ch );
		i3_to_char( "{GNew channel creation and administration:{x\n\r", ch );
		...
		i3_to_char( "{GConfiguration and connection control:{x\n\r", ch );
	      i3_to_char( "i3 connect\n\r", ch );
	      i3_to_char( "i3 disconnect\n\r", ch );
		i3_to_char( "i3 packetdebug\n\r", ch );
...
void I3_who( CHAR_DATA *ch, char *argument )
{
   ...
   if( !argument || argument[0] == '\0' ) 
   {
	i3_to_char( "Get an overview of which mud?\n\r"
	    "(use i3 mudlist to get an overview of the muds available)\n\r", ch );
	return;
   }
   if( ( mud = find_I3_mud_by_name( argument ) ) == NULL ) 
   {
...
void I3_finger( CHAR_DATA *ch, char *argument )
{
   ...
   if( !argument || argument[0] == '\0' ) 
   {
	i3_to_char( "Finger who at which mud?\n\r"
	    "(use i3 mudlist to get an overview of the muds available)\n\r", ch );
	return;
   }
   if( ( ps = strchr( argument, '@' ) ) == NULL ) 
...
void I3_char_login( CHAR_DATA *ch )
{
   ...
   if( !I3_is_connected() )
   {
	if( CH_LEVEL(ch) >= this_mud->adminlevel && i3wait == -2 )
	   i3_to_char( "The Intermud-3 connection is down. Attempts to reconnect were abandoned due to excessive failures.\n\r", ch );
	return;
   }
   if( CH_LEVEL(ch) < this_mud->minlevel )
	return;
...
void I3_loop( void ) 
{
   ...
   if( i3wait == 1 )
   {
	...
	if( I3_socket < 1 )
      ...
      I3_startup_packet();
      i3log( "Resetting channel connections." );
      for( channel = first_I3chan; channel; channel = channel->next )
	{
	   if( !channel->local_name || channel->local_name[0] == '\0' )
		continue;
   ...
   if( FD_ISSET( I3_socket, &exc_set ) )
   {
	...
	i3log( "Exception raised on socket." );
	I3_connection_close( TRUE );
	return;
   }
...
void I3_shutdown( int delay )
{
   ...
   if( I3_socket < 1 )
   {
	i3log( "I3_shutdown was called but no socket existed" );
	return;
   }
   for( channel = first_I3chan; channel; channel = channel->next )
   {
...
char *I3_find_social( CHAR_DATA *ch, char *sname, char *person, char *mud, bool victim )
{
   ...
   {
	i3_printf( ch, "Social %s does not exist on this mud.\n\r", sname );
	return socname;
   }
   if( person && person[0] != '\0' && mud && mud[0] != '\0' )
   {
	 ...
	 && mud && mud[0] != '\0' && !str_cmp( mud, I3_THISMUD ) )
	{
	   ...
	   {
		i3_printf( ch, "Social %s: Missing others_auto.\n\r", sname );
		return socname;
	   }
	...
	else
	{
	   if( !victim )
	   {
		...
		{
		   i3_printf( ch, "Social %s: Missing others_found.\n\r", sname );
		   return socname;
		}
	   ...
	   else
	   {
		...
		{
		   i3_printf( ch, "Social %s: Missing vict_found.\n\r", sname );
		   return socname;
		}
   ...
   else
   {
	...
	{
	   i3_printf( ch, "Social %s: Missing others_no_arg.\n\r", sname );
	   return socname;
	}
...
bool I3_command_hook( CHAR_DATA *ch, char *command, char *argument ) 
{
   ...
   if( !this_mud )
   {
	i3log( "Ooops. I3 called with missing configuration!" );
	return FALSE;
   }
   if( CH_LEVEL(ch) < this_mud->minlevel )
	return FALSE;
   ...
   {
	int x;
	i3_printf( ch, "The last 20 %s messages:\n\r", channel->local_name );
	for( x = 0; x < 20; x++ )
	{
	   if( channel->history[x] != NULL )
   ...
   if( !I3_hasname( I3LISTEN(ch), channel->local_name ) )
   {
	i3_printf( ch, "You were trying to send something to an I3 "
	   "channel but you're not listening to it.\n\rPlease use the command "
	   "'I3 listen %s' to listen to it.\n\r", channel->local_name );
	return TRUE;
   }
   switch( argument[0] )
   {
