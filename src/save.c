...
void fwrite_char (CHAR_DATA * ch, FILE * fp)
{
    ...
    fprintf (fp, "Sex  %d\n", ch->sex);
    fprintf (fp, "Levl %d\n", ch->level);
    fprintf (fp, "God %s~\n", IS_NPC (ch) ? "" : god_table[ch->pcdata->god].name);
    if (ch->trust != 0)
        fprintf (fp, "Tru  %d\n", ch->trust);
    ...
    fprintf (fp, "Exp  %d\n", ch->exp);
    fprintf (fp, "Brownie %d\n", ch->pcdata->browniepoints );
    fprintf(fp, "Quest_Curr %d\n", ch->pcdata->quest_curr );
    if( ch->pcdata->nextquest != 0 )
    	fprintf(fp, "QuestNext %d\n", ch->pcdata->nextquest );
    else if( ch->pcdata->countdown != 0)
    	fprintf(fp, "QuestNext %d\n", 10 );
    /* don't save aggro-safe status! */
    if (IS_SET(ch->act, PLR_SAFE))
    	REMOVE_BIT(ch->act, PLR_SAFE);
    if (ch->act != 0)
        fprintf (fp, "Act  %s\n", print_flags (ch->act));
    if (ch->affected_by != 0)
    ...
    for (;;)
    {
        ...
        switch (UPPER (word[0]))
        {
	    	...
	    	KEY ("Balance", ch->pcdata->balance, fread_number(fp));
				...
				KEY ("Brownie", ch->pcdata->browniepoints, fread_number(fp));
                break;

            case 'C':
	    		KEY ("Cash", ch->cash, fread_number(fp));
            ...
            case 'G':
	    		KEY ("God", ch->pcdata->god, god_lookup(fread_string(fp)));
                KEY ("Gold", ch->cash, fread_number (fp));
		...
		break;
            ...
            case 'S':
                ...
                KEY ("Silv", ch->cash, fread_number (fp));
...
void fread_pet (CHAR_DATA * ch, FILE * fp)
{
    ...
    for (;;)
    {
        ...
        switch (UPPER (word[0]))
        {
            ...
            case 'C':
	    	KEY ("Cash", pet->cash, fread_number (fp));
            ...
            case 'G':
                KEY ("Gold", pet->cash, fread_number (fp));
                break;
            ...
            case 'S':
                ...
                KEY ("Silv", pet->cash, fread_number (fp));
                break;
